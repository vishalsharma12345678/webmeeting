...
document
  .getElementById("video-off")
  .addEventListener("click", turnUserVideoOnOff);
...
$("form").submit(function (e) {
  ...
  if (inputMsg != "") socket.emit("send-message", inputMsg, userName);
  $("input").val("");
});
...
socket.on("recieve-message", (inputMsg, userName) => {
  ...
  $("#messages").append(
    `<li><b style="font-size:.9rem">${userName}</b>&nbsp;${formatAMPM(
      new Date()
    )}<br/><br/>${inputMsg}</li>`
  );
...
document
  ...
  .addEventListener("click", startScreenShare);
